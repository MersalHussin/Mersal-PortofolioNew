...
const Clients = () => {
  ...
  // Split clients into two rows for dual-lane scrolling
  const mid = Math.ceil(clients.length / 2);
  const row1 = clients.slice(0, mid);
  const row2 = clients.slice(mid);

  // Pause only the hovered row's animation
        ...
        >
          <div className="marquee-track flex w-max animate-marquee-left">
            {[...row1, ...row1, ...row1].map((client, i) => (
              <div key={`r1-${i}`}>{renderCard(client)}</div>
            ))}
          </div>
        </div>
        ...
        >
          <div className="marquee-track flex w-max animate-marquee-right">
            {[...row2, ...row2, ...row2].map((client, i) => (
              <div key={`r2-${i}`}>{renderCard(client)}</div>
            ))}
          </div>
        </div>
      </div>
